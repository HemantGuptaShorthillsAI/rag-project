...
class VectorDB:
    ...
    def setup_collection(self, collection_name="TextChunks"):
        existing_collections = self.client.collections.list_all()
    ...
    def insert_data(self, collection_name, text_chunks, embedding_model):
...
class WeaviateTextEmbedder:
    def __init__(self, folder_path, vector_db, embedding_model, collection_name="TextChunks"):
        self.folder_path = folder_path
    ...
    def run(self):
        self.vector_db.setup_collection(self.collection_name)
        all_texts = self.load_text_files()
        all_chunks = [chunk for text in all_texts for chunk in self.split_text(text)]
        self.vector_db.insert_data(self.collection_name, all_chunks, self.embedding_model)
        print("Embedding and storage in Weaviate completed.")
...
if __name__ == "__main__":
    ...
    vector_db = VectorDB(WEAVIATE_URL, WEAVIATE_API_KEY)
    embedding_model = EmbeddingModel(MODEL_NAME)
    embedder = WeaviateTextEmbedder(folder_path, vector_db, embedding_model)
    embedder.run()